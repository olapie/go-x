...
func (m M) ContainsBool(key string) bool {
	_, err := xconv.ToBool(m.Get(key))
	return err == nil
}

func (m M) Bool(key string) bool {
	v, _ := xconv.ToBool(m.Get(key))
	return v
}

func (m M) DefaultBool(key string, defaultValue bool) bool {
	if v, err := xconv.ToBool(m.Get(key)); err == nil {
		return v
	}
	return defaultValue
}
...
func (m M) Int(key string) int {
	v, _ := xconv.ToInt64(m.Get(key))
	return int(v)
}

func (m M) DefaultInt(key string, defaultVal int) int {
	if v, err := xconv.ToInt64(m.Get(key)); err == nil {
		return int(v)
	}
	return defaultVal
}
...
func (m M) IntSlice(key string) []int {
	l, _ := xconv.ToIntSlice(m.Slice(key))
	return l
}

func (m M) ContainsInt64(key string) bool {
	_, err := xconv.ToInt64(m.Get(key))
	return err == nil
}
...
func (m M) ContainsFloat64(key string) bool {
	_, err := xconv.ToFloat64(m.Get(key))
	return err == nil
}

func (m M) Float64(key string) float64 {
	v, _ := xconv.ToFloat64(m.Get(key))
	return v
}

func (m M) DefaultFloat64(key string, defaultValue float64) float64 {
	if v, err := xconv.ToFloat64(m.Get(key)); err == nil {
		return v
	}
	return defaultValue
}
...
func (m M) BigInt(key string) *big.Int {
	...
	if k, err := xconv.ToInt64(m[key]); err == nil {
		return big.NewInt(k)
	}
...
func (m M) BigFloat(key string) *big.Float {
	...
	if k, err := xconv.ToFloat64(m[key]); err == nil {
